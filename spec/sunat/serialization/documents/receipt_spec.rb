...
describe 'serialization of a receipt' do
  ...
  context 'without accounting_customer_party' do
    ...
    it 'should show have cbc:CustomerAssignedAccountID tag with a -' do
      accounting_customer_party_tag = @xml.xpath('//cbc:CustomerAssignedAccountID[text()="-"]')
      expect(accounting_customer_party_tag.count).to eql 1
    end

    it 'should show have cbc:AdditionalAccountID tag with a -' do
      accounting_customer_party_tag = @xml.xpath('//cbc:AdditionalAccountID[text()="-"]')
      expect(accounting_customer_party_tag.count).to eql 1
    end
  end
end
